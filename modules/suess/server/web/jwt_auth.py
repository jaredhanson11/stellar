...
class SummJwtException(BaseException):
    pass
...
class JWTManager:
    ...
    def load_user(self, id: str) -> object:
        ...
        raise SummJwtException("load_user method was never set on JWTManager object.")

    def load_admin_user(self, id: str) -> object:
        """Implement this function to enable JWT admin protected endpoints"""
        raise SummJwtException(
            "load_admin_user method was never set on JWTManager object."
        )
    ...
    def default_load_user_fn(session, user_model):
        ...
        def load_user(id):
            return session.query(user_model).filter_by(login_id=id).one_or_none()
    ...
    def expire_token(self, id: str) -> None:
        ...
        raise SummJwtException(
            "expire_token method was never set on JWTManager object."
        )
    ...
    def requires_auth(self, f):
        ...
        def decorator(*args, **kwargs):
            ...
            token = (
                request.headers.get(self.jwt_header_name) if token is None else token
            )
            ...
            if curr_user:
                ...
                logger.debug(
                    "Login failed because: token=%s, jwt_data=%s, jwt_id=%s, curr_user=%s",
                    token,
                    jwt_data,
                    jwt_id,
                    curr_user,
                )
                raise SuessException(401, "Token authentication failed")
    ...
    def requires_fresh_auth(self, f):
        ...
        def decorator(*args, **kwargs):
            ...
            if curr_user and jwt_data.get("fresh", None):
                ...
                raise SuessException(401, "Fresh authentication failed")
            return f(*args, **kwargs)
    ...
    def requires_admin_auth(self, f):
        """
        Requires non-expired JWT token in the request cookie/header.
        Token must correspond to a valid amin user, defined by the load_admin_user function.
        """
        ...
        def decorator(*args, **kwargs):
            ...
            jwt_data = self._decode_jwt(token) if token else None
            jwt_id = jwt_data.get("id") if jwt_data else None
            curr_user = self.load_admin_user(jwt_id) if jwt_id else None
            if curr_user:
                g.user = curr_user
            else:
                logger.debug(
                ...
                raise SuessException(401, "Admin authentication failed")
            return f(*args, **kwargs)
    ...
    def encode_jwt(self, id: str) -> str:
        ...
        jwt_data = {"iat": int(datetime.utcnow().timestamp()), "id": id}

        return jwt.encode(
            jwt_data, self.secret_key, algorithm=self.jwt_algorithm
        ).decode("utf-8")
